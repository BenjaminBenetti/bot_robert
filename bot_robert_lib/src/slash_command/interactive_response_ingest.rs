...
pub async fn process_action(response: &SlackFormPayloadTransfer) -> Result<(SlackResponse, Option<String>), Box<dyn Error>>{
...
async fn handle_block_actions_response(payload: &String) -> Result<(SlackResponse, Option<String>), Box<dyn Error>>{
...
async fn process_block_action(slack_block_actions: SlackBlockActions) -> (SlackResponse, String) {
    ...
    for action in &slack_block_actions.actions {
        for processor in &command_processors {
            if processor.can_handle_command(&action.action_id) {
                return (processor.handle_block_actions_command(&slack_block_actions).await.unwrap_or(SlackResponse::from_string("eh?")), slack_block_actions.response_url)
            }
        }
    }
